        ...
        try {
            ...
            if ($InputObject) {
                ...
                $install = @(
                    ...
                    '$moduleRoot = Join-Path -Path $destination -ChildPath "{0}"'
                ...
                $install = $install -f $InputObject.Name
...
function Get-BuildItem {
    ...
    try {
        ...
        if ($Type -eq 'ShouldMerge') {
            ...
            [String[]]$exclude = $itemTypes.Values + '*.config', 'test*', 'doc*', 'help', '.build*.ps1', 'buildConfig.psd1'
...
task CreateChocoPackage {
    if ($buildInfo.Config.CreateChocoPackage -eq $true -and (Get-Command choco -ErrorAction SilentlyContinue)) {
        ...
        $script = {
            ...
            Set-Item Function:ConvertTo-ChocoPackage -Value $using:function
        ...
        $function = Get-Content Function:ConvertTo-ChocoPackage
