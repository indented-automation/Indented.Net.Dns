function Get-Dns {
  ...
  if ($NSSearch) {
    ...
    $AuthoritativeServerList = $DnsResponse.Answer | Where-Object RecordType -eq ([Indented.DnsResolver.RecordType]::NS) | ForEach-Object {
      ...
      $NameServerIP = $DnsResponse.Additional |
        Where-Object { $_.Name -eq $NameServer.Hostname -and ($_.RecordType -eq [Indented.DnsResolver.RecordType]::A -or $_.RecordType -eq [Indented.DnsResolver.RecordType]::AAAA) } |
        Select-Object -ExpandProperty IPAddress 
      if ($NameServerIP) {
  ...
  if ($RecordType -eq [Indented.DnsResolver.RecordType]::IXFR -and -not $SerialNumber) {
    # SerialNumber is required, throw an error and abandon this.
    $ErrorRecord = New-Object Management.Automation.ErrorRecord(
  ...
  if ($ZoneTransfer -and $psboundparameters.ContainsKey("SerialNumber")) {
    ...
    $RecordType = [Indented.DnsResolver.RecordType]::IXFR
    $DnsResponse = Get-Dns $Name -RecordType $RecordType -Server $Server -SerialNumber $SerialNumber -NoSearchList
    
    if ($DnsResponse.Header.RCode -eq [Indented.DnsResolver.RCode]::NoError) {
      if ($DnsResponse.Answer[0].Serial -le $SerialNumber) {
        ...
        if ($DnsResponse.Header.ANCount -eq 1 -and $DnsResponse.Answer[0].RecordType -eq [Indented.DnsResolver.RecordType]::SOA) {
    ...
    $RecordType = [Indented.DnsResolver.RecordType]::AXFR
    $Tcp = $true
  ...
  if (-not $Iterative -and -not $NSSearch -and -not $ZoneTransfer) {
    $SearchStatus = [Indented.DnsResolver.RCode]::NXDomain; $i = 0
    ...
    do {
      ...
      # Construct a message
      if ($RecordType -eq [Indented.DnsResolver.RecordType]::IXFR -and $SerialNumber) {
      ...
      if (-not $NoEDns -and -not ($RecordType -in ([Indented.DnsResolver.RecordType]::AXFR), ([Indented.DnsResolver.RecordType]::IXFR))) {
        $DnsQuery.SetEDnsBufferSize($EDnsBufferSize) 
      }
      if ($DnsSec -and -not ($RecordType -in ([Indented.DnsResolver.RecordType]::AXFR), ([Indented.DnsResolver.RecordType]::IXFR))) {
        $DnsQuery.SetAcceptDnsSec()
      }
      
      if ($NoRecursion) {
        # Recursion is set by default, toggle the flag.
        $DnsQuery.Header.Flags = [Indented.DnsResolver.Flags]([UInt16]$DnsQuery.Header.Flags -bxor [UInt16][Indented.DnsResolver.Flags]::RD)
      }
      ...
      do {
        ...
        if ($Tcp) {
          ...
          if ($MessageBuffer.Count -ge 2) {
            ...
            while ($MessageBuffer.Count -ge ($MessageLength + 2)) {
              ...
              if ($RecordType -eq [Indented.DnsResolver.RecordType]::IXFR -and -not $MessageComplete) {
                if ($ActiveSerialNumber -eq $null) {
                  ...
                  if ($DnsResponse.Header.ANCount -ge 2) {
                    if ($DnsResponse.Answer[1].RecordType -ne [Indented.DnsResolver.RecordType]::SOA) {
                ...
                if ($ActiveSerialNumber) {
                  $SOAResouceRecordCount += $DnsResponse.Answer |
                    Where-Object { $_.RecordType -eq [Indented.DnsResolver.RecordType]::SOA -and $_.Serial -eq $ActiveSerialNumber } |
                    Measure-Object |
              ...
              if ($RecordType -eq [Indented.DnsResolver.RecordType]::AXFR -and -not $MessageComplete) {
                $SOAResouceRecordCount += $DnsResponse.Answer |
                  Where-Object RecordType -eq ([Indented.DnsResolver.RecordType]::SOA) |
                  Measure-Object |
                  Select-Object -ExpandProperty Count
        ...
        if ($DnsResponse) {
          ...
          # Return the response
          if ($SearchStatus -ne [Indented.DnsResolver.RCode]::NXDomain -or $DnsDebug) {
            if ($DnsCacheReverse.Contains($DnsResponse.Server)) {
              $DnsResponse.Server = "$($DnsResponse.Server) ($($DnsCacheReverse[$DnsResponse.Server]))"
            }
       
            if ($DnsResponse.Header.Flags -band [Indented.DnsResolver.Flags]::TC) {
              if ($NoTcpFallback) {
                $DnsResponse
              } else {
        ...
        if ($SearchStatus -eq [Indented.DnsResolver.RCode]::NXDomain -and $i -eq ($SearchList.Count - 1)) {
          if ($RecordType -in ([Indented.DnsResolver.RecordType]::AXFR), ([Indented.DnsResolver.RecordType]::IXFR)) {
    ...
    } while ($SearchStatus -eq [Indented.DnsResolver.RCode]::NXDomain -and $i -lt $SearchList.Count)
  }
}